...
public class LayerGroveSpriteLeaves extends RenderLayer<EntityGroveSprite, ModelGroveSprite> {
   ...
   public LayerGroveSpriteLeaves(RenderGroveSprite spriteRendererIn, EntityRendererProvider.Context context) {
      ...
      this.spriteModel = new ModelGroveSprite(context.bakeLayer(RenderGroveSprite.LEAVES_LAYER), true);
   }
   ...
   public void render(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, EntityGroveSprite entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
      ...
      ResourceLocation texture = entitylivingbaseIn.isCinderSprite() ? GROVELEAF_CINDER_TEXTURES : GROVELEAF_TEXTURES;
